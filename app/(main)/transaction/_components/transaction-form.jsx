...
const AddTransactionForm = ({
  ...
  const handleScanComplete = (scannedData) => {
    if (scannedData) {
      ...
      // 1. AI se mile category NAAM ko dhoondho
      if (scannedData.category) {
        // 2. Apne categories ki list mein se us naam wali category ko dhoondho
        const matchedCategory = categories.find(
          (cat) => cat.name.toLowerCase() === scannedData.category.toLowerCase()
        );

        // 3. Agar category mil gayi, toh uski ID ko form mein set karo
        if (matchedCategory) {
          setValue("category", matchedCategory.id);
        } else {
          toast.error(
            `Category "${scannedData.category}" not found. Please select one manually.`
          );
        }
      }
    }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          // Important: We add a value prop here to make the dropdown controlled
